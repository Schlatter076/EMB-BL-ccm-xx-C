 ...
 ***
 *
 * _NOTE_: The FNV-0 historic hash is not recommended.  One should use
...
typedef dsU32   Fnv32_t;  // 32 bit FNV-0 hash type
 ...
 *
 * _NOTE_: The \'s above are not back-slashing escape characters.
 * They are literal ASCII  backslash 0x5c characters.
 *
 * _NOTE_: The FNV-1a initial basis is the same value as FNV-1 by definition.
 */
...
typedef struct fnv0_32_test_vector
{
    testVector_t  *test;    /* test vector buffer to hash */
    Fnv32_t       fnv0_32;  /* expected FNV-0 32 bit hash value */
} fnv032TestVector_t;

typedef struct fnv1_32_test_vector
{
    testVector_t  *test;	  /* test vector buffer to hash */
    Fnv32_t       fnv1_32;  /* expected FNV-1 32 bit hash value */
} fnv132TestVector_t;

typedef struct fnv1a_32_test_vector
{
    testVector_t  *test;    /* test vector buffer to hash */
    Fnv32_t       fnv1a_32; /* expected FNV-1a 32 bit hash value */
} fnv1a32TestVector_t;

typedef struct fnv0_64_test_vector
{
    testVector_t  *test;	  /* test vector buffer to hash */
    Fnv64_t       fnv0_64;  /* expected FNV-0 64 bit hash value */
} fnv064TestVector_t;

typedef struct fnv1_64_test_vector
{
    testVector_t  *test;	  /* test vector buffer to hash */
    Fnv64_t       fnv1_64;  /* expected FNV-1 64 bit hash value */
} fnv164TestVector_t;

typedef struct fnv1a_64_test_vector
{
    testVector_t  *test;	  /* test vector buffer to hash */
...
/* test_fnv.c */
extern testVector_t           fnv_test_str[];
extern fnv032TestVector_t     fnv0_32_vector[];
extern fnv132TestVector_t     fnv1_32_vector[];
extern fnv1a32TestVector_t    fnv1a_32_vector[];
extern fnv064TestVector_t     fnv0_64_vector[];
extern fnv164TestVector_t     fnv1_64_vector[];
extern fnv1a64TestVector_t    fnv1a_64_vector[];
...
////////////////////////////////////////////////////////////////////////////////
/// Func_Name   : fnv32GenHash
/// @param      : p_arg
///               @arg  pStr  : string to be transformed
///               @arg  keyLen: the length of the key
///               @arg  pType : what is the strategy? fnv1a32(default), fnv132,
///                             fnv032??
///               @arg  width : maximum is 32bits, you can choose your mask in
///                             bits
/// @return     : the hash value generated
/// @brief      : use width to control the range of the hash generated,
///               collision can not be avoid
/// @author     : kensky
/// @date       : Jun 10, 2014
/// @pre        : add precondition here if needed.
/// @note       : add some important notes here if needed.
////////////////////////////////////////////////////////////////////////////////
Fnv32_t fnv32GenHash (const unsigned char *pKey,
                      unsigned int        keyLen,
                      char                *pType,
                      unsigned int        width);

/***** Example *****************************************************************
volatile Fnv32_t hash;
std::string str;
cout  << "please enter a string, Let's translate it into hash :(\"quit\" to break)"
      << endl;
...
while(cin>>str)
{
  ...
  hash = fnv32GenHash(str.c_str(), 0, 16);
  cout  << "the hash value is :"<<hash<<endl;
  cout  << "please enter a string, Let's translate it into hash :(\"quit\" to break)"
        << endl;
}
*******************************************************************************/
